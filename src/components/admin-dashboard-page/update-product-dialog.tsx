...
import {createFilesFromImagesResponses, isErrorResponse, isHttpResponse} from "@/lib/utils.ts";
import {toast} from "react-hot-toast";
import {usePatchProductMutation} from "@/apis/ProductsAPI.ts";
...
function UpdateProductDialog(props: UpdateProductDialogProps) {
    ...
    const onSubmit: SubmitHandler<ProductRequest> = async (data) => {
        ...
        const response = await patchProductMutation.mutateAsync({
    ...
    useEffect(() => {
        setSizesNames(props.product.sizes.map((size) => size.name));
    }, [props.product.sizes]);

    useEffect(() => {
        async function awaitImagesCreation() {
            const files = await createFilesFromImagesResponses(props.product.images);
            setImages(files);
        }
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="mr-5 ml-5">
                        ...
                        <div className="space-y-4 rounded-md shadow-sm">
                            ...
                            <div>
                                ...
                                <Input
                                    defaultValue={props.product.brand.name}
                                    id="brandName"
                                    {...register("brandName")}
                            ...
                            <div>
                                ...
                                <Input
                                    defaultValue={props.product.category.name}
                                    id="categoryName"
                            ...
                            <div>
                                ...
                                <Input
                                    defaultValue={props.product.gender.name}
                                    id="genderName"
                                    {...register("genderName")}
                        ...
                        <LoadingButton
                            ...
                            isLoading={patchProductMutation.isPending}>
                            Save changes
                        </LoadingButton>
                    </form>
